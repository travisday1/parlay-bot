...
const GAMES = [
    // ===== NBA GAMES =====
    {
        ...
        confidence: {
            awayML: 68, homeML: 32,
            spread: 52, // ATS record is mediocre; tight line
            over: 62, under: 38, // computer projects 227.6
        },
    ...
    },
    {
        ...
        confidence: {
            awayML: 38, homeML: 62,
            spread: 42, // Hornets 10-2 ATS on b2b — dangerous
            over: 45, under: 55,
        },
    ...
    },
    {
        ...
        confidence: {
            awayML: 88, homeML: 12,
            spread: 72, // MEM is gutted
            over: 40, under: 60, // MEM has no scorers
        },
    ...
    // ===== NHL GAMES =====
    {
        ...
        confidence: {
            awayML: 42, homeML: 58,
            spread: 35, // puck line is volatile
            over: 48, under: 52,
        },
...
function getOverallConfidence(game) {
    const maxML = Math.max(game.confidence.awayML, game.confidence.homeML);
    return maxML;
}
...
function createGameCard(game) {
    ...
    // Spread pick display
    const spreadFavAbbr = game.spread.value < 0 ?
        (game.spread.team === game.away.abbr ? game.away.abbr : game.home.abbr) :
        (game.spread.team === game.away.abbr ? game.away.abbr : game.home.abbr);
...
function calculateOverallConfidence(confidences) {
    ...
    // Multiply probabilities and convert back to %
...
function renderFilterBar() {
    ...
    bar.innerHTML = LEAGUES.map(league => {
        ...
        // Only show leagues that have games or are in the "future" set
