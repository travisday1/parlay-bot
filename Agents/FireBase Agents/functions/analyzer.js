...
async function fetchTodaysGames() {
    // Use start-of-day so we capture ALL today's games, even those that started
    // before the pipeline ran (e.g. early March Madness games)
    const todayStart = new Date();
    todayStart.setHours(0, 0, 0, 0);
    const tomorrow = new Date(todayStart);
    tomorrow.setDate(tomorrow.getDate() + 1);

    // Get all games from start of today through end of today
    const games = await query(
        `SELECT * FROM games
         WHERE sport_key = ANY($1)
         AND commence_time >= $2 AND commence_time < $3
         ORDER BY commence_time ASC`,
        [ALL_SPORT_KEYS, todayStart.toISOString(), tomorrow.toISOString()]
    );
