...
async function apiFetch(path, options = {}) {
    const headers = { ...await getAuthHeader(), 'Content-Type': 'application/json', ...(options.headers || {}) };
...
let activeLeagueFilter = 'all'; // persists league filter across re-renders
...
async function signInWithProvider(provider) {
    try {
        ...
        const errorEl = document.getElementById('auth-error');
        if (err.code === 'auth/operation-not-allowed' || err.code === 'auth/popup-closed-by-user') {
...
async function saveSettings() {
    ...
    try {
        ...
        showSettingsMsg('Error saving: ' + err.message, '#f87171');
    }
}
...
async function resetPassword() {
    ...
    try {
        ...
        showSettingsMsg('Error: ' + err.message, '#f87171');
    }
}
...
async function showApp() {
    ...
    // Load data from Cloud SQL via API
    await loadLiveData();
...
// ===== DATA LOADING FROM CLOUD SQL (via Cloud Functions API) =====
async function loadLiveData() {
    try {
        ...
        // Fetch games, picks, and parlays in parallel via Cloud Functions API
        const [gamesResult, picksResult, parlaysResult] = await Promise.allSettled([
            apiFetch(`/games?date=${todayUTC}`),
            apiFetch(`/picks?date=${todayUTC}`),
            apiFetch(`/parlays?date=${todayUTC}`),
        ]);

        let games = gamesResult.status === 'fulfilled' ? (gamesResult.value.data || []) : [];
        let picks = picksResult.status === 'fulfilled' ? (picksResult.value.data || []) : [];
        let parlays = parlaysResult.status === 'fulfilled' ? (parlaysResult.value.data || []) : [];
        
        if (gamesResult.status === 'rejected') console.warn('⚠️ Games fetch failed:', gamesResult.reason);
        if (picksResult.status === 'rejected') console.warn('⚠️ Picks fetch failed:', picksResult.reason);
        if (parlaysResult.status === 'rejected') console.warn('⚠️ Parlays fetch failed:', parlaysResult.reason);

        // Fallback to local date if UTC returned no picks
        if (picks.length === 0 && todayLocal !== todayUTC) {
            const fallbackPicks = await apiFetch(`/picks?date=${todayLocal}`);
            if (fallbackPicks.data?.length > 0) picks = fallbackPicks.data;
        }
        if (parlays.length === 0 && todayLocal !== todayUTC) {
            const fallbackParlays = await apiFetch(`/parlays?date=${todayLocal}`);
            if (fallbackParlays.data?.length > 0) parlays = fallbackParlays.data;
        }
        ...
        console.log(`✅ Loaded ${GAMES.length} games, ${picks.length} picks, ${RECOMMENDED_PARLAYS.length} parlays from Cloud SQL`);
...
// Helper: Get a displayable team/pick label — handles null picked_team for O/U picks
function getPickTeamDisplay(pick) {
    if (pick.picked_team && pick.picked_team !== 'null') return pick.picked_team;
    // For over/under picks, show "Over LINE" or "Under LINE"
    const pt = (pick.pick_type || '').toLowerCase();
    if (pt === 'over') return `Over ${pick.picked_line || ''}`.trim();
    if (pt === 'under') return `Under ${pick.picked_line || ''}`.trim();
    return pick.pick_type ? pick.pick_type.charAt(0).toUpperCase() + pick.pick_type.slice(1) : 'Unknown';
}
...
function transformGames(games, picks) {
    ...
    return supportedGames.map(game => {
        ...
        return {
            ...
            pick: pick ? {
                team: abbreviate(getPickTeamDisplay(pick)),
...
function buildIntelItems(pick, game, recalcConf) {
    ...
    if (pick) {
        ...
        if (conf >= 75) {
            items.push({ icon: '🔒', text: `LOCK — ${conf}% AI confidence on ${getPickTeamDisplay(pick)}` });
        } else if (conf >= 60) {
            items.push({ icon: '💰', text: `VALUE PLAY — ${conf}% confidence on ${getPickTeamDisplay(pick)}` });
        } else {
            items.push({ icon: '🎲', text: `TOSS-UP — ${conf}% confidence on ${getPickTeamDisplay(pick)}` });
        }
        if (pick.rationale) {
...
function renderGames() {
    ...
    // Re-apply league filter if one was active
    if (activeLeagueFilter && activeLeagueFilter !== 'all') {
        filterGames(activeLeagueFilter);
    }
...
function createSlateRow(game, hasPick) {
    ...
    const offBoard = Math.abs(game.moneyline.away) > 10000 || Math.abs(game.moneyline.home) > 10000;

    // Check which picks are already selected
    const awayMLSel = isPickSelected(game.id, 'awayML') ? ' slate-bet-selected' : '';
    const homeMLSel = isPickSelected(game.id, 'homeML') ? ' slate-bet-selected' : '';
    const spreadFavSel = isPickSelected(game.id, 'spreadFav') ? ' slate-bet-selected' : '';
    const spreadDogSel = isPickSelected(game.id, 'spreadDog') ? ' slate-bet-selected' : '';
    const overSel = isPickSelected(game.id, 'over') ? ' slate-bet-selected' : '';
    const underSel = isPickSelected(game.id, 'under') ? ' slate-bet-selected' : '';

    // ML buttons
    const mlSection = offBoard ? '<span class="slate-odds-val">Off board</span>' : `
        <button class="slate-bet-btn${awayMLSel}" onclick="togglePick('${game.id}', 'awayML', '${game.away.abbr} ML', ${game.moneyline.away}, 50)">
            ${game.away.abbr} <span class="slate-bet-odds">${formatOdds(game.moneyline.away)}</span>
        </button>
        <button class="slate-bet-btn${homeMLSel}" onclick="togglePick('${game.id}', 'homeML', '${game.home.abbr} ML', ${game.moneyline.home}, 50)">
            ${game.home.abbr} <span class="slate-bet-odds">${formatOdds(game.moneyline.home)}</span>
        </button>`;

    // Spread buttons
    const spreadSection = game.spread.value === 0 ? '<span class="slate-odds-val">No line</span>' : `
        <button class="slate-bet-btn${spreadFavSel}" onclick="togglePick('${game.id}', 'spreadFav', '${favTeam} ${game.spread.value}', -110, 50)">
            ${favTeam} <span class="slate-bet-odds">${game.spread.value}</span>
        </button>
        <button class="slate-bet-btn${spreadDogSel}" onclick="togglePick('${game.id}', 'spreadDog', '${dogTeam} +${dogValue}', -110, 50)">
            ${dogTeam} <span class="slate-bet-odds">+${dogValue}</span>
        </button>`;

    // O/U buttons
    const ouSection = !game.overUnder.total ? '<span class="slate-odds-val">—</span>' : `
        <button class="slate-bet-btn${overSel}" onclick="togglePick('${game.id}', 'over', 'Over ${game.overUnder.total}', ${game.overUnder.overOdds || -110}, 50)">
            O <span class="slate-bet-odds">${game.overUnder.total}</span>
        </button>
        <button class="slate-bet-btn${underSel}" onclick="togglePick('${game.id}', 'under', 'Under ${game.overUnder.total}', ${game.overUnder.underOdds || -110}, 50)">
            U <span class="slate-bet-odds">${game.overUnder.total}</span>
        </button>`;
    ...
    return `
        <div class="slate-row" data-league="${game.league}" data-id="${game.id}">
            ...
            <div class="slate-matchup">
                <div class="slate-team-block">
                    <span class="slate-team-name">${game.away.name}</span>
                    <span class="slate-team-city">${game.away.city}</span>
                </div>
                <span class="slate-vs">@</span>
                <div class="slate-team-block">
                    <span class="slate-team-name">${game.home.name}</span>
                    <span class="slate-team-city">${game.home.city}</span>
                </div>
                ${pickIndicator}
            </div>
            <div class="slate-bets">
                <div class="slate-bet-group">
                    <span class="slate-bet-label">ML</span>
                    ${mlSection}
                </div>
                <div class="slate-bet-group">
                    <span class="slate-bet-label">PTS</span>
                    ${spreadSection}
                </div>
                <div class="slate-bet-group">
                    <span class="slate-bet-label">O/U</span>
                    ${ouSection}
                </div>
            </div>
        </div>
...
function renderFilterBar() {
    ...
    bar.innerHTML = allLeagues.map(league => {
        ...
        return `<button class="filter-btn ${league.id === activeLeagueFilter ? 'active' : ''}" 
...
function filterGames(league) {
    activeLeagueFilter = league; // Persist the selection
...
async function loadPerformanceData(startDate, endDate) {
    try {
        ...
        const result = await apiFetch(`/results?days=${days}`);
...
async function loadPerformanceFallback(startDate, endDate) {
    // Fallback not needed — API handles the join
    return await loadPerformanceData(startDate, endDate);
}
...
async function loadParlayPerformanceData(startDate, endDate) {
    try {
        const days = Math.ceil((new Date(endDate) - new Date(startDate)) / (1000 * 60 * 60 * 24)) + 1;
        const result = await apiFetch(`/parlays?date=${startDate}`);
        const parlays = (result.data || []).filter(p =>
            p.parlay_date >= startDate && p.parlay_date <= endDate && p.result !== 'pending'
        );
        return parlays;
    } catch (e) {
...
function toggleSidebar() {
    ...
    updateParlayFAB(); // Update FAB visibility when sidebar toggles
}

// ===== FLOATING PARLAY FAB =====
function updateParlayFAB() {
    let fab = document.getElementById('parlay-fab');
    const sidebar = document.getElementById('parlay-sidebar');
    const sidebarOpen = sidebar && sidebar.classList.contains('open');

    if (selectedPicks.length > 0 && !sidebarOpen) {
        if (!fab) {
            fab = document.createElement('button');
            fab.id = 'parlay-fab';
            fab.className = 'parlay-fab';
            fab.onclick = toggleSidebar;
            document.body.appendChild(fab);
        }
        fab.innerHTML = `🎰 My Parlay <span class="fab-badge">${selectedPicks.length}</span>`;
...
function updateTierCounters() {
    ...
    if (allEl) allEl.textContent = `(${PICKED_GAMES.length})`;
